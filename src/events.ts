...
export class NotebookOpenEventProducer {
    ...
    async listen(_: NotebookPanel, router: ITelemetryRouter, logNotebookContentEvent: boolean) {
        if (!this.produced) {
            const event = {
...
const getVisibleCells = (notebookPanel: NotebookPanel) => {
    ...
    for (let index = 0; index < notebookPanel.content.widgets.length; index++) {
        ...
        const viewBottom = notebookPanel.content.node.scrollTop + notebookPanel.content.node.clientHeight;
...
export class NotebookScrollProducer {
    ...
    listen(notebookPanel: NotebookPanel, router: ITelemetryRouter, logNotebookContentEvent: boolean) {
        notebookPanel.content.node.addEventListener('scroll',
            async (e: Event) => {
                e.stopPropagation()
...
export class ClipboardCopyEventProducer {
    ...
    listen(notebookPanel: NotebookPanel, router: ITelemetryRouter, logNotebookContentEvent: boolean) {
        notebookPanel.node.addEventListener('copy',
            async () => {
                const cell = {
...
export class ClipboardCutEventProducer {
    ...
    listen(notebookPanel: NotebookPanel, router: ITelemetryRouter, logNotebookContentEvent: boolean) {
        notebookPanel.node.addEventListener('cut',
            async () => {
                const cell = {
                    id: notebookPanel.content.activeCell?.model.id,
                    index: notebookPanel.content.widgets.findIndex((value) => value === notebookPanel.content.activeCell)
...
export class ClipboardPasteEventProducer {
    ...
    listen(notebookPanel: NotebookPanel, router: ITelemetryRouter, logNotebookContentEvent: boolean) {
        notebookPanel.node.addEventListener('paste',
